...
template <typename T> T std_alg(const Linear::Matrix<T> &matrix);

double gauss(const Linear::Matrix<double> &matrix);
} // namespace Determinant

template <typename T> class Matrix : private detail::MatrixBuffer<T>
{
  private:
    size_t nRows_ = 0;
    size_t nCols_ = 0;
...
template <typename T> std::istream &operator>>(std::istream &stream, Matrix<T> &rhs);
...
template <typename T> Matrix<T> operator-(const Matrix<T> &lhs, const Matrix<T> &rhs);
template <typename T> Matrix<T> operator*(const T number, const Matrix<T> &rhs);
template <typename T> Matrix<T> operator*(const Matrix<T> &lhs, const T number);
template <typename T> Matrix<T> operator*(const Matrix<T> &lhs, const Matrix<T> &rhs);
} // namespace Linear

template <typename T> T Linear::Determinant::std_alg(const Linear::Matrix<T> &matrix)
{
...
template <typename T> void Linear::Matrix<T>::reverse_gauss()
{
    ...
    : Matrix(rows, cols)
{
    ...
    if (size != size_)
        throw std::invalid_argument("Vector size is not matched with matrix size.");
...
template <typename T> void Linear::Matrix<T>::swap(Matrix &rhs) noexcept
{
...
template <typename T> Linear::Matrix<T> &Linear::Matrix<T>::operator=(const Matrix &rhs)
{
    if (this != &rhs)
    {
        Matrix tmp{rhs};
...
template <typename T> Linear::Matrix<T>::Matrix(Matrix &&rhs) noexcept
{
    this->swap(rhs);
}

template <typename T> Linear::Matrix<T> &Linear::Matrix<T>::operator=(Matrix &&rhs) noexcept
{
    if (this != &rhs)
        this->swap(rhs);
...
template <typename T> bool Linear::Matrix<T>::operator==(const Matrix &rhs) const
{
    if (shape() != rhs.shape())
        return false;
...
template <typename T> Linear::Matrix<T> &Linear::Matrix<T>::operator*=(const T number)
{
...
template <typename T> Linear::Matrix<T>::operator std::vector<T>()
{
    std::vector<T> ans{};
    for (size_t i = 0; i < nRows_; ++i)
        for (size_t j = 0; j < nCols_; ++j)
...
template <typename T> void Linear::Matrix<T>::resize(pair shape)
{
...
template <typename T> void Linear::Matrix<T>::transpose()
{
...
template <typename T> void Linear::Matrix<T>::negate()
{
    for (size_t i = 0; i < nRows_; ++i)
        for (size_t j = 0; j < nCols_; ++j)
            at(i, j) = -1 * at(i, j);
...
template <typename T> void Linear::Matrix<T>::clear()
{
    *this = std::move(Matrix<T>{});
}

template <typename T> void Linear::Matrix<T>::diagonalize()
{
    direct_gauss();
    reverse_gauss();
}

template <typename T> int Linear::Matrix<T>::direct_gauss()
{
    int gaussFactor = 1;
...
template <typename T> void Linear::Matrix<T>::make_eye()
{
    diagonalize();
...
template <typename T> void Linear::Matrix<T>::swap_rows(int lhs, int rhs)
{
    if (lhs == rhs)
    {
        return;
...
template <typename T> void Linear::Matrix<T>::add_rows(int src, int dest, T factor)
{
    ...
    {
        for (size_t i = 0; i < nCols_; ++i)
            at(dest, i) += at(src, i) * factor;
    }
}

template <typename T> void Linear::Matrix<T>::swap_cols(int lhs, int rhs)
{
...
template <typename T> void Linear::Matrix<T>::add_cols(int src, int dest, T factor)
{
    ...
    {
        for (size_t i = 0; i < nRows_; ++i)
            at(i, dest) += at(i, src) * factor;
    }
}

template <typename T> const T *Linear::Matrix<T>::data() const noexcept
{
    return data_;
}

template <typename T> Linear::Matrix<T> Linear::Matrix<T>::zeros(int n)
{
    Matrix<T> tmp{n};
...
template <typename T> Linear::Matrix<T> Linear::Matrix<T>::eye(int n)
{
    Matrix<T> tmp{n};
    for (size_t i = 0; i < n; ++i)
        tmp.at(i, i) = static_cast<T>(1);

    return tmp;
}

template <typename T> Linear::pair Linear::Matrix<T>::shape() const
{
    return pair{nRows_, nCols_};
}

template <typename T> int Linear::Matrix<T>::size() const noexcept
{
    return size_;
}

template <typename T> T Linear::Matrix<T>::trace() const
{
    T ans{};
...
template <typename T> const T &Linear::Matrix<T>::at(size_t i, size_t j) const
{
...
template <typename T> void Linear::Matrix<T>::dump(std::ostream &stream) const
{
    stream.precision(2);
    for (size_t i = 0; i < nRows_; ++i)
    {
...
template <typename T> T &Linear::Matrix<T>::at(size_t i, size_t j)
{
    if (i >= nRows_ || j >= nCols_)
        throw std::invalid_argument("Incorrect rows/cols number value.");
    else
...
template <typename T> T Linear::Matrix<T>::determinant(Determinant::Type type) const
{
    switch (type)
    {
...
template <typename T> size_t Linear::Matrix<T>::rank() const
{
    Matrix<T> tmp = *this;
    tmp.make_eye(false);
    int ans = 0;
...
template <typename T> std::istream &Linear::operator>>(std::istream &stream, Matrix<T> &rhs)
{
    int rows = 0, cols = 0;
    stream >> rows;
    if (!stream.good())
        throw std::runtime_error("stream error: invalid argument");
...
template <typename T> std::ostream &Linear::operator<<(std::ostream &stream, const Matrix<T> &rhs)
{
    rhs.dump(stream);
    return stream;
}
...
template <typename T> Linear::Matrix<T> Linear::operator*(const T number, const Matrix<T> &rhs)
{
    Matrix<T> tmp{rhs};
    tmp *= number;
...
template <typename T> Linear::Matrix<T> Linear::operator*(const Matrix<T> &lhs, const T number)
{
    return number * lhs;
}
